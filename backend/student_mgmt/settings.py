...
SECRET_KEY = os.environ.get('DJANGO_SECRET_KEY', 'django-insecure-dev-key-change-in-production-use-env-var')
...
# Simple JWT Settings
# Security note: JWT secret is derived from Django SECRET_KEY
...
SPECTACULAR_SETTINGS = {
    ...
    'DESCRIPTION': 'RESTful API for managing students, teachers, courses, enrollments, and grades',
    'VERSION': '1.0.0',
...
# 1. All passwords are automatically hashed by Django's authentication system
# 2. JWT secret MUST come from environment variable (SECRET_KEY)
# 3. Enable HTTPS in production (set SECURE_SSL_REDIRECT = True)
# 4. Consider rate-limiting the login endpoint to prevent brute-force attacks
